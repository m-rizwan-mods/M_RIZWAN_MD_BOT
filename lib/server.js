...
// ========== REAL PAIRING API FIXED ==========
app.post('/pair', async (req, res) => {
    const { number } = req.body;
    if(!number) return res.status(400).json({ error: 'Number required' });
    if(number.length < 11) return res.status(400).json({ error: 'Use 9234xxxxxxxxx format' });

    const sessionPath = `./sessions/session_${number}`;

    // pehle wala session delete karo taake naya code mile
    ...
    try {
        ...
        // 3 sec wait karke code mango
        setTimeout(async () => {
            try{
                if(!sock.authState.creds.registered){
                    const code = await sock.requestPairingCode(number);
                    const formattedCode = code?.match(/.{1,4}/g)?.join("-"); // A1B2-C3D4
                    if(!res.headersSent) res.json({ code: formattedCode });
                }
            }catch(e){
                if(!res.headersSent) res.status(500).json({ error: 'Code generate failed. Try again' });
            }
        }, 3000);
